...
def make_curr_ins(allins):
    ...
    for index in indices:
        if index['name'] == 'NIFTY 50':
            ...
            break
    return curr_ins, types,segments,exchanges,ticksizes,indices,vix_instrument,nifty_50_instrument
...
def probability_below(fname,opt_ltps):
    ...
    for token in last_options_tick.keys():
        if curr_ins[token]['instrument_type'] in ['CE','PE']:
            expiry = curr_ins[token]['expiry']
            break
...
curr_ins, types,segments,exchanges,ticksizes,indices,vix_instrument,nifty_50_instrument = make_curr_ins(allins)
    ...
    if len(ticklist):
        ...
        if not timestamp.second%freq and timestamp.time() > market_open:
            ...
            try:
                ...
                show_tradelist(ledger, open_positions, {}, timestamp, opt_ltps, xl_put_spreads,xl_call_spreads)
            except:
                pass
