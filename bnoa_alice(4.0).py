...
def make_curr_ins(allins):
    ...
    for key in curr_ins.keys():
        ...
        if curr_ins[key]['instrument_type'] in ['CE', 'PE'] and curr_ins[key]['name'] == insname and curr_ins[key][
            'expiry'] == expiry:
...
def update_ltps(last_options_tick, opt_ltps, opt_ois, bidaskspread):
    ...
    if nifty_50_instrument['instrument_token'] in last_options_tick.keys():
        ...
        if spot:
            ...
            return opt_ltps, opt_ois, bidaskspread
        ...
        try:
            if curr_ins[item]['instrument_type'] in ['CE', 'PE'] and curr_ins[item]['name'] == insname:
                if ll < curr_ins[item]['strike'] < ul:
                    bestbid = last_options_tick[item]['depth']['buy'][0]['price']
                    bestask = last_options_tick[item]['depth']['sell'][0]['price']
                    contractname = str(int(curr_ins[item]['strike'])) + curr_ins[item]['instrument_type']
                    opt_ltps[contractname] = (bestbid + bestask) / 2
                    opt_ois[contractname] = last_options_tick[item]['oi']
                    bidaskspread[contractname] = bestask - bestbid
        except Exception as e:
            print(e)
            pass
    return opt_ltps, opt_ois, bidaskspread
...
while datetime.datetime.now().time() < market_close and s.range('J1').value:
    ...
    try:
        ...
        continue
    opt_ltps, opt_ois, bidaskspread = update_ltps(last_options_tick, opt_ltps, opt_ois, bidaskspread)
    if len(open_positions):
    ...
    if not timestamp.second % freq:
        ...
        put_spreads, call_spreads = adjust_bid_ask(put_spreads, call_spreads, bidaskspread)
