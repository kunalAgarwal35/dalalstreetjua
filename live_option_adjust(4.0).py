...
def probability_below(timestamp, last_options_tick):
    global vixnow
    if not vixnow:
        return
...
def find_trades(open_positions, put_spreads, call_spreads):
    ...
    if len(open_positions) == 0:
        ...
        try:
            ...
            if call_spreads[curcallspr] < call_spread_ev_thresh and max(call_spreads.values()) > 15:
                tradelist[cspr[0]] = -1
                tradelist[cspr[1]] = 1
                for key in call_spreads.keys():
                    if call_spreads[key] == max(call_spreads.values()):
        ...
        if not timestamp.second % freq and timestamp.time() > market_open:
            try:
                ...
                try:
                    ...
                    print('Error 3')
                    pass
                prob_below = probability_below(timestamp, last_options_tick)
            except TypeError:
                print('Error 4')
                continue
