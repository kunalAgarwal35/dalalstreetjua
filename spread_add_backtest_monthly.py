...
sheet = book.sheets['credit_5%_vix']
...
for file in os.listdir(pbelow_dir):
    ...
    for timestamp in timestamplist:
        if timestamp.date() in traded_dates.keys():
            if traded_dates[timestamp.date()] > 9:
                continue
        pbnow = pb[timestamp]
        ...
        try:
            ...
            if not len(pbnow['ltps'])>0 or not len(pbnow['ois'])>0:
                continue
            pbnow['vix'] = fs.get_vix_close(timestamp - datetime.timedelta(minutes=5))
        ...
        # Trade Exclusions
        if days_to_expiry > 40 or days_to_expiry < 5:
            continue
        ...
        nr = {'timestamp': timestamp, 'trade': trade_spread,'spread_type':spread_type,'stance':stance, 'valmax': valmax, 'result': result,
              'days_to_expiry': days_to_expiry, 'expiry': expiry, 'vix': pbnow['vix'], 'distance_from_put': distance_from_put,
    ...
    '''
# plot_series = result_df['result'][result_df['stance'] == 'Bearish'][result_df['distance_from_call']>2*result_df['distance_from_put']]
