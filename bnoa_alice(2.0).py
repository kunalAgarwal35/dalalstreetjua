...
def probability_below(timestamp, last_options_tick, opt_ltps):
    ...
    for item in last_options_tick.keys():
        if curr_ins[item]['instrument_type'] in ['CE', 'PE'] and curr_ins[item]['name'] == insname:
            keyname = str(int(curr_ins[item]['strike'])) + curr_ins[item]['instrument_type']
            if keyname in opt_ltps.keys():
                oi[keyname] = last_options_tick[item]['oi']
                if curr_ins[item]['instrument_type'] == 'CE':
                    calls[keyname] = opt_ltps[keyname]
                else:
                    puts[keyname] = opt_ltps[keyname]
...
def show_tradelist(ledger, open_positions, tradelist, timestamp, option_ltps, xl_put_spreads, xl_call_spreads, ts):
    ...
    pnlgraph(timestamp, openpnl, booked_pnl, netpnl, slipped)
