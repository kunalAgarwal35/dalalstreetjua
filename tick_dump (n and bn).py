...
market_close = datetime.time(15, 31, 00)
...
def get_tokens():
    ...
    banknifty_ltp = q['lastPrice']
    all_tradables_nifty=tradables(allins,nifty_ltp*0.9,nifty_ltp*1.1)
    all_tradables_banknifty=tradables_bn(allins,banknifty_ltp*0.9,banknifty_ltp*1.1)
    TOKENS=[]
    for ins in all_tradables_nifty:
        TOKENS.append(ins['instrument_token'])
    for ins in all_tradables_banknifty:
        TOKENS.append(ins['instrument_token'])
...
def tradables(instru, lstrike, ustrike):
    ...
    return filtered
def tradables_bn(instru, lstrike, ustrike):
    global trading_sessions
    filtered = []
    types = ['CE', 'PE', 'FUT']
    expfut = 100
