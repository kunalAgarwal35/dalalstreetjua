...
percentile = 75
dayformat="%y-%m-%d"
options_historical_dir = 'findata-master/NIFTY/options/'
...
def initiate_variables():
    if 'ledger_n' in os.listdir(aux_files):
        ledger = pickle.load(open(aux_files + 'ledger_n', 'rb'))
    else:
        ledger = pd.DataFrame()

    if 'open_positions_n' in os.listdir(aux_files):
        open_positions = pickle.load(open(aux_files + 'open_positions_n', 'rb'))
    else:
        open_positions = {}
...
def square_off_all():
    ...
    tradelist = {}
    for key in open_positions.keys():
        tradelist[key] = -open_positions[key]/75
    return tradelist
    ...
    try:
        ...
        if timestamp.time() > market_open:
            ...
            if len(tradelist) > 0:
                ...
                payoff_graph(open_positions, opt_ltps)
                pickle.dump(open_positions, open(aux_files + 'open_positions_n', 'wb'))
                pickle.dump(ledger, open(aux_files + 'ledger_n', 'wb'))
