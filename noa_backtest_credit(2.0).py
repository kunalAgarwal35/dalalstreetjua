...
def display_sdandmean(timestamp):
    ...
    try:
        ...
        expiry = find_expiry(timestamp)
        trading_sessions = int(find_trading_sessions(timestamp, expiry))
...
def spread_evs(prob_below, opt_ltps):
    ...
    for i in range(0, len(calls) - 2):
        for j in range(i + 1, len(calls) - 1):
            ...
            if buy_strike in prob_below.keys() and sell_strike in prob_below.keys():
...
def check_adjustment_allowed(timestamp):
    global trade_stop_time
    if timestamp.date() == find_expiry(timestamp) and timestamp.time() > trade_stop_time:
        return 0
    else:
        return 1
...
#     expopt = 100
#     for ins in allins:
#         if ins['name'] == 'BANKNIFTY' and ins['instrument_type'] in types:
...
def update_ltps(last_options_tick, opt_ltps,opt_ois):
    ...
    for item in last_options_tick.keys():
        if item not in curr_ins.keys():
            print(item,'Not in curr_ins')
            continue
        try:
...
#Set new = 0 to initiate all variables from scratch
new = 1
opt_ltps = {}
opt_ois = {}
ol_tradelist = {}
spot = 0
ledger, open_positions,pnldf = initiate_variables(new)
last_processed = ''
s.range('J1').value = 1
...
for item in dayticks:
    ...
    for timestamp in tickdict.keys():
            ...
            for item in open_positions.keys():
                if item not in opt_ltps.keys():
                    if 'CE' in item:
                        ...
                        puts_present = 0
            if (not (calls_present*puts_present)) and not (timestamp.date() == find_expiry(timestamp) and timestamp.time()>sq_off_time):
                continue
        ...
        if timestamp.date() == find_expiry(timestamp) and timestamp.time() > sq_off_time:
            if len(open_positions):
                tradelist = square_off_all()
                ledger, open_positions = show_tradelist_sq(ledger, open_positions, tradelist, timestamp, opt_ltps,{}, {}, spot)
        ...
        if not timestamp.second % freq:
            prob_below = pbh.find_pbelow(timestamp)
            ...
            if not check_adjustment_allowed(timestamp):
                tradelist = {}
            if len(tradelist):
                if len(ol_tradelist):
