...
def show_tradelist_sq(ledger, open_positions, tradelist, timestamp, option_ltps, xl_put_spreads, xl_call_spreads,spot):
    ...
    sqpnl = 0
    for key in open_positions.keys():
        sqpnl += expiry_price[key] * open_positions[key]
...
for item in dayticks:
    ...
    for timestamp in tickdict.keys():
        ...
        try:
            try:
                if timestamp <= last_processed or timestamp.second == last_processed.second:
                    continue
            except:
                pass
            ...
            try:
                ...
                pass
            if timestamp.date() == find_expiry(timestamp) and timestamp.time() > datetime.time(15,10,0):
                if len(open_positions):
