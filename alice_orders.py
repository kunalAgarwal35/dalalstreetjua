...
access_token = AliceBlue.login_and_get_access_token(username=uname, password=passw, twoFA='abc',  api_secret=apis, app_id = appid)
...
def find_expiry():
    ...
    expopt = 100
    for ins in allins:
        if ins['name'] == 'NIFTY' and ins['instrument_type'] in types and ins['expiry'] >= datetime.datetime.now().date():
...
def process_nifty_tradelist(tradelist):
    calls = {}
    puts = {}
...
def process_banknifty_tradelist(tradelist):
    calls = {}
    puts = {}
    for key in tradelist.keys():
        if 'CE' in key:
