...
insname = 'NIFTY'
lot_size = 75
#Minimum ev required to take a new trade at 3 pm
min_net_ev = 25
...
def spread_evs(prob_below, opt_ltps):
    ...
    for i in range(0, len(calls) - 2):
        for j in range(i + 1, len(calls) - 1):
            ...
            if name_to_strike(buykey) not in prob_below.keys() or name_to_strike((sellkey)) not in prob_below.keys():
                continue
...
# Set new = 0 to start initiate variables from scratch
new = 0
opt_ltps = {}
opt_ois = {}
ol_tradelist = {}
...
if 'lp_ev_test' + aux_name in os.listdir(aux_files) and new:
    last_processed = pickle.load(open(aux_files + 'lp_ev_test' + aux_name, 'rb'))
if 'noda_result' + aux_name in os.listdir(aux_files) and new:
    noa_result = pickle.load(open(aux_files + 'noda_result' + aux_name, 'rb'))
for item in dayticks:
    ...
    for timestamp in tickdict.keys():
        ...
        if timestamp in pbelow.keys():
            ...
            if net_result < min_net_ev:
                if m_projected_calls < 0:
                    net_ev -= m_projected_calls
                    net_result = -m_realized_calls
                if m_projected_puts < 0:
                    net_ev -= m_projected_puts
                    net_result = -m_realized_puts
            ...
            pickle.dump(noa_result, open(aux_files + 'noda_result' + aux_name, 'wb'))
