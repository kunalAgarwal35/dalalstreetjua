...
optionshistoricaldir = '2021options/'
iupac = "%m%d%Y-%H%M%S-%f"
iupac = "%m%d%Y-%H%M%S-%f"
...
freq = 6
manual_range = [15300, 16300]
# % of spot up and down to limit trading strikes
trade_range = 0.03
# strike range from spot to consider for probability model
prob_range = 0.1
dayformat="%y-%m-%d"
...
def tradable_strikes():
    global last_options_tick,trade_range,spot
    try:
...
def initiate_variables():
    if 'ledger_noatb' in os.listdir(aux_files):
        ledger = pickle.load(open(aux_files + 'ledger_noatb', 'rb'))
    else:
        ledger = pd.DataFrame(columns = ['expiry','instrument','oid','price','qty','value'])

    if 'open_positions_noatb' in os.listdir(aux_files):
        open_positions = pickle.load(open(aux_files + 'open_positions_noatb', 'rb'))
    else:
        open_positions = {}
...
def find_trades(open_positions, put_spreads, call_spreads):
    ...
    if len(open_positions) == 0:
        ...
        try:
            ...
            if call_spreads[curcallspr] < call_spread_ev_thresh:
                tradelist[cspr[0]] = -1
                tradelist[cspr[1]] = 1
                for key in call_spreads.keys():
                    if call_spreads[key] == max(call_spreads.values()):
        ...
        try:
            ...
            if put_spreads[curputspr] < put_spread_ev_thresh:
                tradelist[pspr[0]] = -1
                tradelist[pspr[1]] = 1
                for key in put_spreads.keys():
                    if put_spreads[key] == max(put_spreads.values()):
...
def find_expiry(timestamp):
    t1 = time.time()
    for item in os.listdir(optionshistoricaldir):
...
def show_tradelist(ledger, open_positions, tradelist, timestamp, opt_ltps, xl_put_spreads, xl_call_spreads, ts):
    expiry = find_expiry(timestamp)
    nrlist = []
    lot_size = 75
    ...
    for nr in nrlist:
        ...
        nr['exchange_timestamp'] =  datetime.datetime.now()
    ...
    print_ledger['exp_ins'] = print_ledger['instrument'] + ' ' + str(print_ledger['expiry'])
    sqpnl = 0
    for key in open_positions.keys():
    ...
    booked_pnl = {}
    poplist = []
    for contract in print_ledger['exp_ins']:
        if contract not in booked_pnl.keys():
            if print_ledger[print_ledger['exp_ins'] == contract]['qty'].sum() == 0:
                booked_pnl[contract] = print_ledger[print_ledger['exp_ins'] == contract]['value'].sum()
        else:
            if print_ledger[print_ledger['exp_ins'] == contract]['qty'].sum() != 0:
...
def update_ltps(last_options_tick, opt_ltps):
    global spot,nifty_50_instrument
    try:
...
print('Initiating')
ol_tradelist = {}
trading_days = os.listdir('dayoptionticks')[1:]
if 'last_processed_noatb' in os.listdir(aux_files):
    last_processed = pickle.load(open(aux_files+'last_processed_noatb','rb'))
for day in trading_days:
    ...
    for timestamp in day_ticks.keys():
        if not s.range('J1').value:
            break
        try:
            if timestamp.second % freq or timestamp.second == last_processed.second:
                continue
        except:
            pass
        if not timestamp.second % 58:
            pickle.dump(timestamp, open(aux_files + 'last_processed_noatb', 'wb'))
            opt_ltps = {}
        try:
        ...
        try:
            ...
            if timestamp.date() == find_expiry(timestamp) and timestamp.time() > datetime.time(15,20,0):
                if len(open_positions):
                    ...
                    ledger, open_positions = show_tradelist_sq(ledger, open_positions, tradelist, timestamp, opt_ltps,[], [], ts)
                else:
                    continue
            if not vixnow:
                try:
            ...
            if not timestamp.second % freq:
                ...
                if len(prob_below):
                    try:
                        ...
                        print('Error in calculating spread EV')
                        continue
                else:
                    continue
                try:
                    ...
                    if len(tradelist):
                        if len(ol_tradelist):
                            if ol_tradelist == tradelist:
                                ...
                                pickle.dump(open_positions, open(aux_files + 'open_positions_noatb', 'wb'))
                                pickle.dump(ledger, open(aux_files + 'ledger_noatb', 'wb'))
                                ol_tradelist = {}
                            else:
