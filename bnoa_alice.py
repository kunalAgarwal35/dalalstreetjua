...
tdate = datetime.datetime(2021,7,16)
...
def probability_below(timestamp, last_options_tick, opt_ltps):
    ...
    for item in last_options_tick.keys():
        if curr_ins[item]['instrument_type'] in ['CE', 'PE'] and curr_ins[item]['name'] == 'BANKNIFTY':
            ...
            if curr_ins[item]['name'] == 'NIFTY BANK':
...
def initiate_variables():
    if 'ledger_banknifty' in os.listdir(aux_files):
        ledger = pickle.load(open(aux_files + 'ledger_banknifty', 'rb'))
    else:
        ledger = pd.DataFrame(columns = ['expiry','instrument','oid','price','qty','value'])

    if 'open_positions_banknifty' in os.listdir(aux_files):
        open_positions = pickle.load(open(aux_files + 'open_positions_banknifty', 'rb'))
    else:
        open_positions = {}
        ...
        try:
            if curr_ins[item]['instrument_type'] in ['CE', 'PE'] and curr_ins[item]['name'] == 'BANKNIFTY':
                if ll < curr_ins[item]['strike'] < ul:
                    opt_ltps[str(int(curr_ins[item]['strike'])) + curr_ins[item]['instrument_type']] = (
                                                                                                                   last_options_tick[
                                                                                                                       item][
                                                                                                                       'depth'][
                                                                                                                       'buy'][
                    ...
                    if len(ol_tradelist):
                        if ol_tradelist == tradelist:
                            ledger, open_positions = show_tradelist(ledger, open_positions, tradelist, timestamp,
                                                                    opt_ltps,
                                                                    xl_put_spreads, xl_call_spreads, ts)
                            pickle.dump(open_positions, open(aux_files + 'open_positions_banknifty', 'wb'))
                            pickle.dump(ledger, open(aux_files + 'ledger_banknifty', 'wb'))
                            ol_tradelist = {}
                        else:
