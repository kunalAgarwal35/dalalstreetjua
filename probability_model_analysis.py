...
import pickle
import numpy as np
import function_store_backup as fs
...
for timestamp in pb.keys():
    if sdate < timestamp <todate:
        data = pb[timestamp]
        distnow = normal_distribution_cdfable(mean=data['historical_mean'], sd=data['historical_sd'])
        ...
        if timestamp.hour == 10 and timestamp.minute == 0:
            s.range('A1').value = df
            print_distribution(timestamp,data)
        print(timestamp)
        opt_ltps = data['last_price']
        opt_ois = data['oi']
        spot = data['spot']
        l = 0.97*spot
        u = 1.03*spot
        mean = data['historical_mean']
        sd = data['historical_sd']
