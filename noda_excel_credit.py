...
# nsename = 'nifty bank'
nsename = 'nifty 50'
adjustment_time = datetime.time(15,19,0)
def pingdiscord(txt):
...
def tradable_strikes():
    global trade_range,nsename
    nse = Nse()
    # print(nse)
    # index_codes = nse.get_index_list()
    q = nse.get_index_quote(nsename)
    spot = q['lastPrice']
    try:
...
def copy_and_sort_spreads(put_spreads, call_spreads):
    t1 = time.time()
    # Uncomment to monitor both credit and debit spreads
...
def display_xl_spreads(xl_put_spreads,xl_call_spreads):
...
def check_adjustment_allowed():
    global adjustment_time,ledger
    now = datetime.datetime.now()
    if 'timestamp' in ledger.columns:
        tslist = ledger['timestamp'].to_list()
        tslist = [i.date() for i in tslist]
        tslist = list(set(tslist))
    else:
        tslist = []
    if now.time()>adjustment_time and now.date() not in tslist:
        return 1
    else:
        return 0
...
while datetime.datetime.now().time() < market_close and s.range('J1').value:
    ...
    if not timestamp.second % freq:
        ...
        display_xl_spreads(xl_put_spreads,xl_call_spreads)
        put_spreads, call_spreads = copy_and_sort_spreads(put_spreads, call_spreads)
        if check_adjustment_allowed():
